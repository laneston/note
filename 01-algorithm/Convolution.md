- [卷积神经网络在图像处理中的基本原理](#卷积神经网络在图像处理中的基本原理)
  - [局部感知机制（Local Receptive Fields）](#局部感知机制local-receptive-fields)
  - [参数共享机制（Weight Sharing）](#参数共享机制weight-sharing)
  - [分层特征抽象（Hierarchical Feature Learning）](#分层特征抽象hierarchical-feature-learning)
    - [特征提取流程：](#特征提取流程)
    - [组合机制：](#组合机制)
  - [多通道特征融合（Multi-Channel Fusion）](#多通道特征融合multi-channel-fusion)
  - [非线性激活增强（Non-Linear Activation）](#非线性激活增强non-linear-activation)
  - [空间层次结构（Spatial Hierarchy）](#空间层次结构spatial-hierarchy)
...
卷积计算之所以能有效捕获图像特征，主要基于以下几个关键机制：

## 局部感知机制（Local Receptive Fields）

- 核心思想：卷积核每次仅处理输入图像的局部区域（如3×3或5×5的窗口）
- 生物学依据：模仿人类视觉系统的感受野机制（视网膜细胞仅对局部光信号敏感）
- 作用效果：
  - 检测局部特征（如边缘、角点、纹理）
  - 忽略全局无关信息，聚焦关键区域
...
## 参数共享机制（Weight Sharing）

- 核心思想：同一卷积核在整个图像上滑动使用相同的权重参数
- 优势：
  - 显著减少参数量（相比全连接网络）
  - 保证特征检测的平移不变性（同一特征在不同位置被同等检测）
- 计算示例：
  - 输入图像尺寸：224×224
  - 3×3卷积核的参数量：3×3×C_in×C_out（C为通道数）
  - 全连接层的参数量：224×224×C_in × 输出维度（通常大几个数量级）


## 分层特征抽象（Hierarchical Feature Learning）
...
## 非线性激活增强（Non-Linear Activation）

关键组件：在卷积后添加激活函数（如ReLU）
...
Output=max(0,ConvOutput)


## 空间层次结构（Spatial Hierarchy）

通过堆叠卷积层和下采样（如池化层）逐步实现：
