...
池化层通过缩小特征图的尺寸（如从 28×28 降到 14×14），显著减少后续层的参数量和计算量。例如，最大池化（Max Pooling）取局部区域的最大值，平均池化（Average Pooling）取平均值。

增强特征不变性

**平移不变性：**池化操作对输入的小幅平移、旋转或形变不敏感。例如，最大池化会保留局部最显著的特征（如边缘、纹理），即使目标位置稍有偏移，输出特征仍保持稳定。

**鲁棒性：**通过忽略局部细节的微小变化，模型更关注整体特征，提升对噪声和干扰的鲁棒性。

**加速训练与推理：**数据维度降低后，网络对硬件资源的需求减少，训练和推理速度更快。

**减少参数数量：**通过降维间接降低了模型复杂度，缓解过拟合风险。池化层本身没有可训练参数，仅通过固定规则压缩数据。

**更高层次的抽象：**池化后的特征图每个像素对应原始输入中更大的区域（感受野），使后续层能捕捉更全局的语义信息（如物体部件或整体结构）。
...
功能：将前层提取的局部或空间特征（如卷积层输出的特征图）整合为全局语义信息。

示例：在图像分类中，将卷积层检测到的“边缘”“纹理”等局部特征，组合成“车轮”“车窗”等高级语义，最终判断是否为“汽车”。

映射到任务目标

分类任务：通过全连接层将特征映射到类别概率（如输出10维向量表示10个数字的概率）。

回归任务：直接预测连续值（如坐标、价格）。

参数密集，增强模型容量

全连接层通过大量可学习参数，赋予网络更强的非线性拟合能力，适合学习复杂模式。
...
全连接：每个神经元与前一层的所有神经元相连，公式为：$y=Wx+b$ ，其中 W 为权重矩阵，x 为输入向量，b 为偏置。

维度变换

输入：特征图展平后的向量（如卷积层输出的 7×7×512 展平为 25088 维）。

输出：任务所需维度（如分类任务的类别数）。

激活函数

通常搭配非线性激活函数（如ReLU）增强表达能力，但输出层根据任务选择：
